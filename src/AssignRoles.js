...
function AssignRoles() {
    ...
    const loadBlockchainData = async () => {
        ...
        if (contract_address /*networkData*/) {
            ...
            let traCount;
            try {
                traCount = await contract.methods.traCtr().call();
                console.log('Transaction Count:', traCount);
            } catch (error) {
                console.error('Error fetching transaction count:', error);
            }
            //const traCount = await contract.methods.traCtr().call();
            ...
            const tra = await Promise.all(Array(parseInt(traCount)).fill().map((_, i) => contract.methods.TRA(i + 1).call()));
        ...
        try {
            ...
            switch (type) {
                case "rms":
                    ...
                    break;
                case "tra":
                    receipt = await supplyChain.methods.addTransporter(address, name, place).send({ from: currentAccount });
                    break;
                case "man":
    ...
    return (
        <div className="assign-roles-container">
            ...
            {["rms", "tra","man", "dis", "ret"].map((roleType) => (
                <div key={roleType}>
